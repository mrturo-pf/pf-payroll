...
from payroll.application.dto import ComputeIncomeTaxCommandDTO, ComputeIncomeTaxResultDTO, IncomeTaxContextDTO
...
async def test_compute_income_tax_uses_stored_utm_and_persists_result() -> None:
    ...
    result = await ComputeIncomeTax(repository, market_data_repository).execute(ComputeIncomeTaxCommandDTO(period_id=10))
...
async def test_compute_income_tax_rejects_missing_utm_value() -> None:
    with pytest.raises(ValueError, match="UTM exchange rate for 2026-01-31 was not found."):
        await ComputeIncomeTax(StubPayrollRepository(), StubMarketDataRepository(None)).execute(
...
async def test_compute_income_tax_rejects_missing_bracket() -> None:
    ...
    with pytest.raises(ValueError, match="No income tax bracket was found"):
        await ComputeIncomeTax(MissingBracketRepository(), StubMarketDataRepository()).execute(
            ComputeIncomeTaxCommandDTO(period_id=10)
        )
