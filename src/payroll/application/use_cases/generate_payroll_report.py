...
from payroll.application.errors import PayrollConflictError, PayrollPeriodNotFoundError, PayrollSummaryNotFoundError
...
class GeneratePayrollReport:
    ...
    async def execute(self, period_id: int) -> GeneratedPayrollReportDTO:
        ...
        if detail.status != "reviewed":
            raise PayrollConflictError(f"Payroll period {period_id} must be reviewed before generating a report.")
        if detail.summary is None:
            raise PayrollSummaryNotFoundError(f"Payroll summary for period {period_id} was not found.")
