...
| `GET` | `/payroll/period-range` | Returns the current payroll period plus 12 previous and 12 next period date ranges. The current period is the latest payroll with `declared_net_pay_clp` present and `payment_date <= today`; when none exists, the endpoint falls back to the current calendar month and infers missing periods from the configured employer payment rule or the default last Chilean business day of month. |
