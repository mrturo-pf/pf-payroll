...
| `POST` | `/payroll/import/pdf-preview` | Read-only: extracts a preview of the payroll rows found in an uploaded PDF payslip, using versioned templates (see `infrastructure/pdf_import/templates/`). Never persists anything and never fails outright -- a PDF matching no known template still returns 200 with every row marked unresolved (`concept_code: null`). `payment_date` is resolved from the matched employer's real configured payment-date rule (a read-only lookup -- "never persists" only ever meant no writes) when that employer is already registered; falls back to a generic "last business day of the month" guess otherwise, which a human can still correct by hand before `POST /payroll/import/rows`. |
...
| `template-test <pdf>` | Previews a payroll PDF against the current templates without persisting anything (no DB access at all, unlike `POST /payroll/import/pdf-preview`) -- prints which template matched (if any) and which rows are still unresolved. `payment_date` is always the generic "last business day of the month" guess here, never the employer's real configured rule, since this command has no database to read it from. Helper for building/adjusting `infrastructure/pdf_import/templates/*.json` by hand. |
