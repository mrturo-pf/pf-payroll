...
def test_market_data_endpoints() -> None:
    ...
    try:
        exchange_rates = client.get(
            "/market-data/exchange-rates", params={"currency_code": "UF"}
        )
        economic_indices = client.get(
            "/market-data/economic-indices", params={"code": "IPC_CL"}
        )
...
def test_market_data_refresh_endpoint_surfaces_domain_errors() -> None:
    ...
    try:
        response = client.post(
            "/market-data/refresh", json={"exchange_rates": [], "economic_indices": []}
        )
    finally:
        app.dependency_overrides.clear()
