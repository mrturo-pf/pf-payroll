...
class ComputeIncomeTax:
    ...
    async def execute(
        ...
        taxable_base_clp = max(
            Decimal("0"), context.taxable_income_clp - context.deductible_amount_clp
        )
        taxable_base_utm = (
        ...
        if bracket is None:
            raise IncomeTaxBracketNotFoundError(
                ...
                "for "
                f"{context.payment_date.isoformat()} and taxable base "
                f"{taxable_base_utm} UTM."
            )
